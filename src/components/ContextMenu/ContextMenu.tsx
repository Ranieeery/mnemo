...
const ContextMenu: React.FC<ContextMenuProps> = ({
    ...
    return (
        ...
        >
            ...
            <button
                onClick={() => {
                    onOpenWith(video.file_path);
                    onClose();
                }}
                className={buttonClass}
            >
                <svg
                    className={`${iconClass} text-yellow-400 group-hover:text-yellow-300 group-active:text-yellow-200`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                >
            ...
            <button onClick={handleToggleWatchedStatus} className={buttonClass}>
                {video.is_watched ? (
                    <svg
                ...
                <span className="font-medium">{video.is_watched ? "Mark as Unwatched" : "Mark as Watched"}</span>
            </button>

            <button
                onClick={() => {
                    onOpenProperties(video);
                    onClose();
                }}
